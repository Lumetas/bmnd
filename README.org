...
* Быстрый старт
  ...
  \BMND\Core\Router\Router::setup('app/Controllers', 'App\\Controllers\\')->run();
  #+END_SRC
...
* Middleware
  ** Создание middleware
     ...
     use BMND\Core\Http\MiddlewareInterface;
     use BMND\Core\Http\RequestInterface;
     use BMND\Core\Http\ResponseInterface;
     use BMND\Core\Http\RequestHandlerInterface;
...
* Расширенные примеры
  ** API контроллер
     ...
     class ApiController
     {
     	#[Route('/users', 'GET', 'api.users.index')]
     	public function index(UserService $service)
     	{
     		return new Response($service->all());
     	}

     	#[Route('/users/{id}', 'GET', 'api.users.show')]
...
* FAQ
  ** PSR
     Да, RequestInterface/ResponseInterface совместимы с PSR-7
